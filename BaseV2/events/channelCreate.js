...
module.exports = (client, channel) => {
  ...
  setTimeout(async () => {
    if (channel.topic !== null) {
      ...
      let msg = `__**${user.tag}'s Warnings**__`;
      let expPoints = 0;
      let expMsg = '';
      let curPoints = 0;
      let curMsg = '';
      ...
      infractions.forEach((i) => {
        ...
        if ((i.points * 604800000) + i.date > time) {
          curPoints += i.points;
          curMsg += `\n• Case ${i.case} - ${moderator ? `Mod: ${moderator.tag}` : `Unknown Mod ID: ${i.moderator || 'No ID Stored'}`} - (${moment.utc(i.date).format('DD MMM YYYY')} UTC) ${i.points} warn${i.points === 1 ? '' : 's'}\n> Reason: ${i.reason}`;
        } else {
          expPoints += i.points;
          expMsg += `\n• Case ${i.case} - ${moderator ? `Mod: ${moderator.tag}` : `Unknown Mod ID: ${i.moderator || 'No ID Stored'}`} - (${moment.utc(i.date).format('DD MMM YYYY HH:mm')} UTC) ${i.points} warn${i.points === 1 ? '' : 's'}\n> Reason: ${i.reason}`;
        }
      });
      ...
      if (curMsg || expMsg) {
        ...
        // No infractions
        channel.send(`${user.tag} doesn't have any warns!`);
      }
    }
  }, 2000);
};
