...
module.exports = (client) => {
  if (!client.firstReady) {
    ...
    console.log(chalk.magenta('| First ready event triggered, loading the guild.'));
    const intv = setInterval(() => {
      const guild = client.guilds.cache.first();
      if (!guild) {
        console.log(chalk.red(`| Attempting to wait for guild to load ${counter}...`));
        counter += 1;
        return;
      }
      clearInterval(intv);
      console.log(chalk.green('| Guild successfully loaded.'));
      ...
      setInterval(() => {
        ...
        client.user.setActivity(`Watching over ${guild.memberCount} users!`);
      }, 30000);
      ...
      console.log(chalk.blue(`| Ready sequence finished, with ${guild.memberCount} users, in ${guild.channels.cache.size} channels of ${client.guilds.cache.size} guilds. Bot is running! Checking for errors...`));
      setTimeout(() => {  console.log(chalk.green("| No Errors detected Bot ready to comply!")); }, 10000);
    }, 1000);
  } else {
